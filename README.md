...
Claude Code was used for the A-MoreOOP increment: turning `HappyBot` into an object and extracting the `Ui`, `Parser`, and `TaskList` classes out of it in five small steps, running the console UI test cases after each one.
It also explained the design choices involved, such as when a private helper method earns its place and why the `due` command has to carry each deadline's number from the full task list.
