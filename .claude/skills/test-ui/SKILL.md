---
name: test-ui
description: Run the console UI test cases in test/ui-test-plan.md and compare each session's output against the expected output. Use after every code update, and when asked to test command-line interactions or verify console output against the test plan.
---

# Test UI

Runs the console UI test cases defined in `test/ui-test-plan.md`. Each case holds
...
1. Update `test/ui-test-plan.md` first, so its build command, run command, and
